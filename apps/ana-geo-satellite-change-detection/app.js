...
function pickScene(featureId, role) {
  ...
  err('');
  saveState((s) => { s.scenes[role] = scene; }).then(() => {
    // 선택 → 칩 (§24.1). 검증을 통과해 실제로 슬롯에 들어간 장면만 칩이 된다.
    addChip(`scene:${role}:${scene.id}`, `${role === 'before' ? '⏮' : '⏭'} ${role}: ${scene.id}`, { type: 'scene', role, id: scene.id });
    renderSlots(); renderScenes(); renderLayers();
  });
}
