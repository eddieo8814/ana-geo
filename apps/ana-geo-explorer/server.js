#!/usr/bin/env node
// ana-geo-explorer server — ANA Geo shared runtime contracts (PRD §8.2–8.5).
...
// §8.4 — per-app external host allowlist. The browser never calls Overpass
// directly; every query goes through POST /api/proxy against this list.
const ALLOWED_HOSTS = ['overpass-api.de'];
const USER_AGENT = 'ana-geo-explorer/0.1 (ANA Geo; https://github.com/tykimos/ana-geo)';
...
// Overpass is queried with POST (`data=<QL>`), so the request content-type and
// body are forwarded too. The upstream status and body are passed through
// verbatim: Overpass answers 200 with an HTML error page when it is rate
// limiting, and the client — not the proxy — decides success by parsing (§25).
async function handleProxy(req, res, url) {
  ...
  if (isPost) headers['content-type'] = req.headers['content-type'] || 'application/x-www-form-urlencoded';
  try {
...
server.listen(PORT, () => {
  console.log(`ana-geo-explorer → http://localhost:${PORT}  (state: ${path.basename(STATE_FILE)})`);
});
