#!/usr/bin/env node
// ana-geo-map server — ANA Geo shared runtime contracts (PRD §8.2–8.5).
...
// §8.4 — per-app external host allowlist. ana-geo-map needs no external API.
const ALLOWED_HOSTS = [];
...
async function handleProxy(req, res, url) {
  ...
  try {
    const upstream = await fetch(parsed, { method: req.method === 'POST' ? 'POST' : 'GET', headers, body: req.method === 'POST' ? await readBody(req) : undefined });
    const buf = Buffer.from(await upstream.arrayBuffer());
    // content-length must reflect the buffer we actually send: fetch auto-decompresses
    // gzip, so the upstream header describes the compressed body and would truncate ours.
    const h = { 'content-type': upstream.headers.get('content-type') || 'application/octet-stream', 'content-length': buf.length };
...
server.listen(PORT, () => {
  console.log(`ana-geo-map → http://localhost:${PORT}  (state: ${path.basename(STATE_FILE)})`);
});
