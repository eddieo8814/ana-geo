// Overpass acquisition for candidate destinations (FR-ROUTE-008) — query
// building, failure classification, and OSM → GeoJSON normalization (§11.1).
//
// Every request from the browser goes through the server proxy (PRD §8.4); the
// page never calls overpass-api.de directly. The Python worker reaches the same
// host over direct HTTPS under the same allowlist, which is why `overpass-api.de`
// appears exactly once in server.js and is passed into every worker call.
...
(function (root, factory) {
  ...
  function buildQuery(categoryKey, bbox, limit) {
    const cat = registry.get(categoryKey);
    if (!cat) throw new Error(`unknown category: ${categoryKey}`);
    const [w, s, e, n] = bbox.map((x) => Number(x).toFixed(6));
    const box = `(${s},${w},${n},${e})`;
    const parts = [];
    for (const set of cat.tags) {
      const f = tagFilter(set);
      parts.push(`  node${f}${box};`, `  way${f}${box};`, `  relation${f}${box};`);
    }
  ...
  function classify(status, contentType, bodyText) {
    ...
    // first: the text heuristics below must not fire on a cafe whose name
    ...
    if (/timed out|timeout/.test(head)) {
      return { ok: false, kind: 'timeout', message: 'Overpass query timed out — narrow the area or the category.' };
    }
    ...
    if (status === 504) {
      return { ok: false, kind: 'timeout', message: 'Overpass gateway timeout (504) — narrow the area or the category.' };
    }
  ...
  function toFeature(el, categoryKey, fetchedAt) {
    const tags = el.tags || {};
    const base = {
      type: 'Feature',
      id: `${el.type}/${el.id}`,
    ...
    if (el.type === 'way' && Array.isArray(el.geometry) && el.geometry.length) {
      ...
      const areal = (registry.get(categoryKey) || {}).areal;
      if (areal && isClosed(coords)) {
  ...
  function toGeoJSON(osmJson, categoryKey, cap) {
    const fetchedAt = new Date().toISOString();
    const features = [];
    let truncated = false;
    for (const el of osmJson.elements || []) {
      if (features.length >= cap) { truncated = true; break; }
      const f = toFeature(el, categoryKey, fetchedAt);
  ...
  async function fetchCategory(categoryKey, bbox, options) {
    const opts = options || {};
    const cap = opts.cap || 2000;
    const proxy = opts.proxyPath || '/api/proxy';
    const query = buildQuery(categoryKey, bbox, cap);
    ...
    const { collection, truncated, fetchedAt } = toGeoJSON(verdict.data, categoryKey, cap);
    return { ok: true, collection, truncated, fetchedAt, query, cap };
  }

  return { ENDPOINT, buildQuery, classify, toGeoJSON, toFeature, fetchCategory };
});
